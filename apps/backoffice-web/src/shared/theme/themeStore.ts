...
// Own storage key/app — same shape as apps/web's themeStore.ts (own file,
// not shared, per CLAUDE.md's "Why Backoffice is a separate deploy": no
// shared runtime code between the two web apps), but a distinct key so a
// staff member who is also a professional never has one app's theme choice
// leak into the other, even if they ever shared an origin.
const STORAGE_KEY = 'agendya-backoffice-theme';

function systemTheme(): Theme {
  // `matchMedia` is missing in jsdom (Vitest's test environment) and in any
  // environment that predates it, so this can't assume it exists just
  // because `window` does.
...
/** Applies the theme to the document so tailwind.css's `:root[data-theme]`
 * block can react to it. No Moon `.dark-theme` class to mirror here — this
 * app doesn't use Moon Design System (see CLAUDE.md). */
